...
public class Selection {
  ...
  public Selection(int roomId, int hostId, String roomType, String country, String city, String neighborhood,
  ...
  public Selection(int roomId, int hostId, String roomType, String country, String city, String neighborhood,
      ...
      long tempoExecucao, long repeticoes) {
    this.roomId = roomId;
    this.hostId = hostId;
    this.roomType = roomType;
    this.country = country;
  ...
  public Selection clone() {
    return new Selection(this.roomId, this.hostId, this.roomType, this.country, this.city, this.neighborhood,
   ...
   */
  public static void lerArquivo(String arquivo, Selection[] acomodacoes) throws IOException {
    try (BufferedReader lerArquivo = new BufferedReader(new FileReader(arquivo))) {
      ...
      while ((line = lerArquivo.readLine()) != null && index < acomodacoes.length) {
        Selection acomodacao = new Selection();
        acomodacao.ler(line);
   ...
   */
  public static void trocarPosicao(Selection[] acomodacoes, int i, int j) {
    Selection temp = acomodacoes[i];
   ...
   */
  public static void ordenar(Selection[] acomodacoes, int n) {
    int i, j, menor;
    for (i = 0; i < n - 1; i++) {
      menor = i;
      for (j = i + 1; j < n; j++) {
  ...
  public static void filtrarAcomodacoes(Selection[] acomodacoes, int numQuartos, Selection[] quartosFiltrados,
      Scanner scanner) {
    for (int i = 0; i < numQuartos; i++) {
      String busca = scanner.next();
      for (Selection acomodacao : acomodacoes) {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Selection[] acomodacoes = new Selection[contarLinhasArquivo(arquivo)];
    lerArquivo(arquivo, acomodacoes);

    numQuartos = scanner.nextInt();
    Selection[] quartosFiltrados = new Selection[numQuartos];

    filtrarAcomodacoes(acomodacoes, numQuartos, quartosFiltrados, scanner);
    ordenar(quartosFiltrados, numQuartos);

    for (Selection acomodacao : quartosFiltrados) {
      acomodacao.imprimir();
    }
