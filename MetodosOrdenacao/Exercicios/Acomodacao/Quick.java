...
public class Quick {
  ...
  public Quick(int roomId, int hostId, String roomType, String country, String city, String neighborhood,
      int reviews,
   ...
   */
  public Quick clone() {
    return new Quick(this.roomId, this.hostId, this.roomType, this.country, this.city, this.neighborhood,
   ...
   */
  public static void lerArquivo(String arquivo, Quick[] acomodacoes) throws IOException {
  ...
  public static void trocarPosicao(Quick[] acomodacoes, int i, int j) {
    Quick temp = acomodacoes[i];
    acomodacoes[i] = acomodacoes[j];
    acomodacoes[j] = temp;
  }

  public static int particionar(Quick[] acomodacoes, int comeco, int fim) {
    Quick pivot = acomodacoes[fim];
    int i = comeco - 1;
  ...
  public static void ordenar(Quick[] acomodacoes, int comeco, int fim) {
    int part;
    if (comeco < fim) {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Quick[] acomodacoes = new Quick[contarLinhasArquivo(arquivo)];
    lerArquivo(arquivo, acomodacoes);

    int numQuartos = scanner.nextInt();
    Quick[] quartosFiltrados = new Quick[numQuartos];

    for (int i = 0; i < numQuartos; i++) {
      String busca = scanner.next();
      for (Quick acomodacao : acomodacoes) {
    ...
    for (Quick acomodacao : quartosFiltrados) {
      acomodacao.imprimir();
    }
    scanner.close();
  }
