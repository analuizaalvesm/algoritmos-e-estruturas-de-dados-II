...
public class Heap {
  ...
  public Heap(int roomId2, int hostId2, String roomType2, String country2, String city2, String neighborhood2,
  ...
  public static void sort(Heap[] arr, int n) {
  ...
  static void heapify(Heap[] arr, int n, int i) {
    int largest = i;
    int l = 2 * i + 1;
    int r = 2 * i + 2;
  ...
  public static void main(String[] args) throws IOException {
    ...
    try (BufferedReader lerAirbnb = new BufferedReader(new FileReader(arquivo))) {
      ...
      while ((line = lerAirbnb.readLine()) != null && index < quartos.length) {
        Heap AcomodacaoHeap = new Heap();
        AcomodacaoHeap.ler(line);
        quartos[index] = AcomodacaoHeap;
        index++;
      }
      lerAirbnb.close();
      int numeroQuartos = sc.nextInt();
      Heap[] quartosFiltrados = new Heap[numeroQuartos];
      for (int i = 0; i < numeroQuartos; i++) {
        String pesquisa = sc.next();
        for (Heap AcomodacaoHeap : quartos) {
          if (AcomodacaoHeap.roomId == Integer.parseInt(pesquisa)) {
            quartosFiltrados[i] = AcomodacaoHeap;
          }
        }
      }
      sort(quartosFiltrados, numeroQuartos);

      for (Heap AcomodacaoHeap : quartosFiltrados) {
        AcomodacaoHeap.imprimir();
      }
