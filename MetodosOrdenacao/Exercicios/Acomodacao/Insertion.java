...
public class Insertion {
    ...
    public Insertion(int roomId2, int hostId2, String roomType2, String country2, String city2,
    ...
    public static void sort(Insertion[] array, int n) {
        for (int i = 1; i < n; i++) {
            Insertion tmp = array[i];
            int j = i - 1;
    ...
    public static void main(String[] args) throws IOException {
        ...
        Insertion[] quartos = new Insertion[127992];
        try (BufferedReader lerAirbnb = new BufferedReader(new FileReader(arquivo))) {
            ...
            while ((line = lerAirbnb.readLine()) != null && index < quartos.length) {
                Insertion acomodacao = new Insertion();
                acomodacao.ler(line);
            ...
            Insertion[] quartosFiltrados = new Insertion[numeroQuartos];
            for (int i = 0; i < numeroQuartos; i++) {
                String pesquisa = sc.next();
                for (Insertion AcomodacaoInsertion : quartos) {
                    if (AcomodacaoInsertion.roomId == Integer.parseInt(pesquisa)) {
                        quartosFiltrados[i] = AcomodacaoInsertion;
                    }
                }
            }
            sort(quartosFiltrados, numeroQuartos);

            for (Insertion AcomodacaoInsertion : quartosFiltrados) {
                AcomodacaoInsertion.imprimir();
            }
